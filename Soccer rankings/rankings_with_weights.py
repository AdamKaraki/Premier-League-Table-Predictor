# Adam Karaki
# CPTS 315
# Final Project: Soccer prediction rankings with percentages 
# of their chances of winning the league
# File Description: This file was meant to mess around with the home and away weights
#                   However the logic was inconsistent since not all teams performed worse away from their 
#                   home stadium.
# This gives us the same result as Colley.py


import numpy as np 
import pandas as pd
# Importing packages numpy and pandas to help read in data and create tables

# Reading 'teams' and 'scores' data respectively from the
# text files to put into the algorithm
...
top_teams = r.argsort()[-20:][::-1]
for index,i in enumerate(top_teams):
    print ( str(index+1) + ". " + str(teams.iloc[i][1])+ " \t\t"+str(r[i]))